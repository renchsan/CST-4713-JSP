...
import coreservlets.LongLivedCookie;

@WebServlet("/single-page-form")
public class SinglePageForm extends HttpServlet {
	@Override
		    ...
		    while(paramNames.hasMoreElements()) {
		      ...
		      if (paramName.equals("fname")) {
		    	  if (paramValues.length == 1) {
		    		  param1 = ServletUtilities.filter(paramValues[0]);
		    		  out.println("Received PARAM1");
			    	  Cookie fnameCookie =
			    		        new Cookie("fname", param1);
			    		      fnameCookie.setMaxAge(60*60*24*365); // 1 year
			    		      response.addCookie(fnameCookie);
			    	out.println(fnameCookie);
		  	      } else {
			    	  param1 = "Unknown";
			      }
		      }
		      if (paramName.equals("lname")) {
		    	  if (paramValues.length == 1) {
			  	      param2 = ServletUtilities.filter(paramValues[0]);
			    	  Cookie lnameCookie =
		      ...
		      if (paramName.equals("email")) {
		    	  if (paramValues.length == 1) {
		    		  param3 = ServletUtilities.filter(paramValues[0]);
			    	  Cookie emailCookie =
